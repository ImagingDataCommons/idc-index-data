...
│   └── _version.py         # Version file (auto-generated by hatch-vcs)
...
**1. IDC Data Update (new dataset version):**

- Triggers when IDC releases a new dataset (e.g., v23 → v24)
- Use: `nox -s bump -- --commit`
- Creates tag: `24.0.0` (major version = IDC version)
- Updates SQL scripts and test expectations
- Regenerates parquet files from new BigQuery dataset

**2. Package-Only Update (same IDC data):**

- Bug fixes, documentation, or code improvements
- Manually bump patch version: `git tag -a 23.0.4 -m "Fix bug XYZ"`
- Major version stays the same (still using v23 data)

#### Version Resolution

- Version is determined by git tags (via hatch-vcs)
- `_version.py` is auto-generated during build (not tracked in git)
- No manual version editing in `pyproject.toml` (uses `dynamic = ["version"]`)
